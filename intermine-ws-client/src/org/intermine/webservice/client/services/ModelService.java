...
/**
 * This class represents a connection to the RESTful resource on an InterMine
 * server which provides information about the service's data model. This is a
 * structure which is serialised and transmitted in XML, and contains
 * information about the tables within the database in a highly abstract manner.
 * The data model is required for constructing queries with, in order to
...
public class ModelService extends Service {
	...
	/**
	 * Returns the model used by the InterMine instance which the service is
	 * connected to.
	 *
	 * @return model
	 */
	public Model getModel() {
		...
		if (model == null) {
			...
			// Rather than adding the model by its name (which might overwrite
			// other models of the
			// same name (eg. the default genomic model), we store it by its
			// full URI, meaning
			// pathqueries can refer to their model as either "genomic",
			// or fully qualified as "http://myhost:8080/mymine/models#genomic".
			// This could be
			// useful to distinguish between generic genomic queries that will
			// work on multiple
	...
	protected String getModelXml() {
		Request request = new RequestImpl(RequestType.GET, getUrl(), ContentType.TEXT_PLAIN);
